...
%% ── Таблиця значень ─────────────────────────────────────────
fprintf('\n=== z1(x,y)=sin(x)*sin(y)  [ значення у точках МФ ] ===\n');
fprintf('%10s','');
...
title('Еталонна поверхня  z_1 = sin(x)\cdotsin(y)','FontSize',12);
...
sgtitle('МФ входу x  —  z_1(x,y) = sin(x)\cdotsin(y)');
...
title('z_1 = sin(x)\cdotsin(y)  — Порівняння похибок');
grid on;
for k=1:3
    text(k-wb/2,err_full(k)+.05,sprintf('%.1f%%',err_full(k)),...
         'HorizontalAlignment','center','FontSize',9,'FontWeight','bold');
    text(k+wb/2,err_diag(k)+.05,sprintf('%.1f%%',err_diag(k)),...
...
fprintf('\n╔══════════════════════════════════════════════════════════╗\n');
fprintf('║   ВИСНОВКИ  z1(x,y) = sin(x)*sin(y)                    ║\n');
fprintf('╠══════════════════════════════════════════════════════════╣\n');
